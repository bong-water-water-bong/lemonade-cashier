...
def test_closing_window_adds_factor():
    inputs = RiskInputs(cart=Cart(), voids_in_txn=0, tender=None, closing_hour_local=23)
    result = score(inputs)
...
def test_score_is_capped_at_one():
    ...
    inputs = RiskInputs(cart=cart, voids_in_txn=5, tender="500.00", closing_hour_local=0)
